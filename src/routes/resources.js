...
router.get("/", async (req, res, next) => {
  try {
    ...
    const [rows] = await db.query(
      "SELECT resource_id, resource_name, resource_type, is_active FROM resources",
    );
    ...
    if (rows.length === 0) {
      return res
        .status(404)
        .json({ message: "No resources currently available." });
    }
  ...
  async (req, res, next) => {
    try {
      ...
      if (!location || location.trim() === "") {
        return res
          .status(400)
          .json({ error: "Location is required for new resources." });
      }
